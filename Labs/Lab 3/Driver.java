...
public class Driver {
    ...
    private static int testPartOne(Program3 program, String filename) {
        EconomyCalculator calculator = null;

        try{
            calculator = new EconomyCalculator(filename);
        ...
        return program.computeGain();
    }

    private static int testPartTwo(Program3 program, String filename) {

        VibraniumOreScenario vibraniumScenario = null;

        try {
            vibraniumScenario = new VibraniumOreScenario(filename);
        } catch(FileNotFoundException e) {
            System.err.printf("Could not find file: %s\n", filename);
            usage();
        }

        program.initialize(vibraniumScenario);

        return program.computeLoss();
    }
    ...
    public static void main(String[] args) {
        ...
        //willTestPartOne=true;
        //willTestPartTwo=true;
        //filename="./large_inputs/p2/1024.in";
